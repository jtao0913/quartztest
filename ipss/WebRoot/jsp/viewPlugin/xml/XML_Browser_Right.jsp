...
<body>
<form name="ViewXML" method="post" action="<%=basePath%>viewXml.do?method=viewXml">
...
//alert(windowHight);
<%if(ft_type.equals("3")){%>
	document.write("<div align='right' class='PageNext'><iframe align='right' name='sOutline' id='sOutline' onLoad='javascript:{dyniframesize();}' src='<%=basePath%>renderXML.do?method=renderXML&an=<%=strAN%>&pn=<%=DRA_Page%>&channelIDs=<%=strChannelIDs%>&pd=<%=strPD%>&ft_type=<%=ft_type%>&startpage=<%=startpage%>&pagenum=1&windowWidth="+windowWidth+"' width="+windowWidth+" height="+windowHight+" frameborder='0' scrolling='no'></iframe></div>");
<%}else{%>
	document.write("<div align='right' class='PageNext'><iframe align='right' name='sOutline' id='sOutline' onLoad='javascript:{dyniframesize();}' src='<%=basePath%>renderXML.do?method=renderXML&an=<%=strAN%>&channelIDs=<%=strChannelIDs%>&pd=<%=strPD%>&ft_type=<%=ft_type%>&pageIndex=<%=pageIndex%>&CLM_Page=${requestScope.CLM_Page}&DES_Page=${requestScope.DES_Page}&DRA_Page=${requestScope.DRA_Page}&windowWidth="+windowWidth+"' width=830 height=500 frameborder='0' scrolling='no'></iframe></div>");
<%}%>
