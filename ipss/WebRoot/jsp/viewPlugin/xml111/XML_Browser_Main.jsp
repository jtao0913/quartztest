...
<html>
	<head>
 	...
 	<frameset rows="2" cols="*" framespacing="0" frameborder="no"
		border="0">
		<frame
			src="<%=basePath%>xml.do?an=<%=strAN%>&channelIDs=<%=strChannelIDs%>&pd=<%=strPD%>&CLM_Page=<%=CLM_Page%>&DES_Page=<%=DES_Page%>&DRA_Page=<%=DRA_Page%>&ft_type=<%=ft_type%>&pageIndex=<%=pageIndex%>"
