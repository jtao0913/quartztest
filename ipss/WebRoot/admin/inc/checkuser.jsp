...
try //统一用户是否登录时的异常
{
	try{
		...
		{
			throw new  WASException("10010#对不起，您没有登录或者登录超时#"+strErrorFrom+"#未登录或者登录超时（session为空或者session超时）");
		}
		...
		if (!oCheckUserMessage.getUserState())//检验用户是否处于可用状态，如果不可用，更新当前的载线用户列表
		{
	  	 	...
	  	 	throw new WASException("10020#对不起，非法登录或者是已经被管理员拒绝登录#"+strErrorFrom+"#由于非法登录或者被管理员拒绝登录");
		}
	}
	...
	{
		throw new WASException("10001#对不起，在判定用户是否登录时发生普通异常#"+strErrorFrom+"#"+eCheck.getMessage());  
	}
}
catch(WASException eCheckFinalWAS)
{
	throw eCheckFinalWAS;
}
%>