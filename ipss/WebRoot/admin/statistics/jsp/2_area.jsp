...
int intUserID = Integer.parseInt((String)request.getSession().getAttribute("userid"));
	...
	int[] adminIDs=adminmanage.getUserIDs();
	String[] adminNames=adminmanage.getUserNames();
	
	UserManage usermanage=new UserManage(5000,1,adminID);
...
function selScope(scope){
	if(scope=='all'){
		document.stats.all_area_user.disabled=true;
		document.stats.all_area_user.options.length=0;
	}
	if(scope=='area'){
		document.stats.all_area_user.disabled=false;
		document.stats.all_area_user.options.length=0;
		
		<%for(int t=0;t<adminIDs.length;t++){%>
		var adminID='<%=adminIDs[t]%>';
		var adminName='<%=adminNames[t]%>';
		document.stats.all_area_user.options[<%=t%>]=new Option(adminName,adminID);
		<%}%>
	}
	if(scope=='user'){
		document.stats.all_area_user.disabled=false;
		...
		var userID='<%=userIDs[t]%>';
		var userName='<%=userNames[t]%>';
		document.stats.all_area_user.options[<%=t%>]=new Option(userName,userID);
		<%}%>
	}
}
//-->
    	...
    	<table width="100%" bordercolordark="#ffffff" bordercolorlight="#4787b8" border="0" cellpadding="0" cellspacing="0">
        	<tr align="center"> 
			  ...
			  <table width="100%" border="1" cellspacing="0" cellpadding="0" bordercolorlight="#4787b8" bordercolordark="#FFFFFF">
                ...
                <tr>
                  ...
                  <td width="25%" height="24">
					...
					<table width="239" border="0">
                      <tr>
                        <td width="62"><input name="selectScope" type="radio" value="all" id="id_all" checked onClick="selScope('all')">
                          <label for="id_all" style="cursor:hand"/>全部</td>

						<%if(adminID==1){%>  
                        <td width="64"><input name="selectScope" type="radio" value="area" id="id_area" onClick="selScope('area')">
                        <label for="id_area" style="cursor:hand"/>地区</td>
						<%}%>
												
                        <td width="123"><input name="selectScope" type="radio" value="user" id="id_user" onClick="selScope('user')">
                        <label for="id_user" style="cursor:hand"/>单个用户</td>
                      </tr>
                    </table>
				  
				  </td>
                  <td width="51%" height="24">
				  <!--select name="scopelist" style="width:133" disabled>
                    <%for(int i=0;i<adminIDs.length;i++){%>
                    <option value="<%=adminIDs[i]%>"><%=adminNames[i]%></option>
                    <%}%>
                  </select-->
				  
				  <select name="all_area_user" style="width:133" disabled>
                    <%
					/*
				...
				<tr>
				<td><div align="center">时间跨度：</div></td>
				<td colspan="2">
				
					<table width="157" border="0">
                      <tr>
                        <td width="50"><input name="time_span" type="radio" value="year" id="id_year" checked>
                          <label for="id_year" style="cursor:hand"/>
                          年</td>
                        <td width="50"><input name="time_span" type="radio" value="month" id="id_month">
                          <label for="id_month" style="cursor:hand"/>月</td>
                        <td width="50"><input name="time_span" type="radio" value="day" id="id_day">
                        <label for="id_day" style="cursor:hand"/>日</td>
                      </tr>
                    </table>
