...
try	//统一用户权限控制时的异常处理
{
	...
	try //统一用户权限判定时的异常处理
	{
		...
		{
			oPowerCheckWASConfig = new WASConfig();
			application.setAttribute("wasconfig",oPowerCheckWASConfig);
		}
		...
		{
			//没有用户需要查找的权限
			throw new WASException("XXX#对不起，您不具有指定的权限#"+strErrorFrom+"#");
		}
	}
	...
	{
		throw new WASException("XXX#对不起，在判断用户是否具有指定权限的时候发生错误！#"+strErrorFrom+"#"+ePowerCheck.getMessage());
	}
}
catch(WASException ePowerCheckFinalWAS)
{
	UNIException eCheckUNI = new UNIException(ePowerCheckFinalWAS.getErrorNumber(),ePowerCheckFinalWAS.getErrorDetail(),ePowerCheckFinalWAS);
	throw eCheckUNI;
}
%>