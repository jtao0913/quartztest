	...
	<xsl:template match="cn-patent-document">
		<head>
			...
			{ 
				...
				{
					//self.location = "http://www.cnipr.com";
				}
		...
		<body width="800" font-size="5" onload="initial()" onselectstart="return false" onpaste="return false">
			<style>
...
-->
	...
	<xsl:template match="invention-title">
		<table width="520">
			<tr>
				<td align="center">
					<font face="仿宋" size="3" color="Purple">
						<xsl:copy>
							<xsl:apply-templates select="* | text() | br | sub | sup |comment()"/>
						</xsl:copy>
					</font>
				</td>
			</tr>
	...
	<xsl:template match="figure/img">
		<!--object classid="CLSID:106E49CF-797A-11D2-81A2-00E02C015623">
			...
			<embed type="image/gif" negative="no">
				<xsl:attribute name="width"><xsl:value-of select="@wi"/></xsl:attribute>
				<xsl:attribute name="height"><xsl:value-of select="@he"/></xsl:attribute>
